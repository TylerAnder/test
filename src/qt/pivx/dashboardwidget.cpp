    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of MDDN earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 MDDN");
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("MDDN")
    updateDisplayUnit();
}
...
void DashboardWidget::changeChartColors()
{
    ...
    if (isLightTheme()) {
        ...
        gridLineColorX = QColor(22,22,22);
        linePenColorY =  gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair MDDN, zMDDN
