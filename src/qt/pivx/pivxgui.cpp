...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Modden client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Modden"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Modden"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
