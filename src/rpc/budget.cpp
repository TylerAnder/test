...
UniValue preparebudget(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 6)
        throw std::runtime_error(
            ...
            "5. \"MDDN-address\":   (string, required) MDDN address to send payments to\n"
    ...
    if (!pwalletMain->CreateBudgetFeeTX(wtx, proposalHash, keyChange, false)) { // 50 MDDN collateral for proposal
...
UniValue getbudgetprojection(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "    \"PaymentAddress\": \"xxxx\",     (string) MDDN address of payment\n"
