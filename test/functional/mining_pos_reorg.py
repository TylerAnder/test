...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected MDDN and zMDDN supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zMDDNsupply']:
                assert_equal(node['zMDDNsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check MDDN and zMDDN supply at the beginning
        # ------------------------------------------
        # zMDDN supply: 2 coins for each denomination
        ...
        # MDDN supply: block rewards minus burned fees for minting
        ...
        # Verify that MDDN and zMDDN supplies were properly updated after the spends and reorgs
        self.log.info("Check MDDN and zMDDN supply...")
